...
import com.hiphen.generic.objectrepository.EditDocSpecizlization;
...
public class EditSpecialization extends BaseClass {
	...
	WebElement edit = driver.findElement(By.xpath("(//td[text()='"+specialization+"'])/ancestor::tr/descendant::a[@class='btn btn-transparent btn-xs']"));
	wlib.scrollToBottomJs(driver);
	wlib.moveToElementAndClick(driver, edit);
	EditDocSpecizlization eds=new EditDocSpecizlization(driver);
	String username = specialization+"123";
	eds.getEditDocSpecialization().clear();
	eds.getEditDocSpecialization().sendKeys(username);
	eds.getUpdateButton().click();
	String res = driver.findElement(By.xpath("//p[contains(text(),'Doctor Specialization updated successfully !!')]")).getText();
